...
DROP PROCEDURE IF EXISTS `pr_cms_get_tchqnew_rpt`$$
CREATE PROCEDURE  `pr_cms_get_tchqnew_rpt`(IN In_Condition Longtext)
BEGIN
   set @rownum=0;
	set @v_query=concat('select @rownum := @rownum + 1 as ''Sl No'', a.chq_date as ''Cheque Date'',
                a.acc_holder_name as ''A/c Holder'',
                a.chq_no as ''Cheque No'',
                a.chq_amount as ''Cheque Amount'',
                a.micr_code as ''Micr Code'',
                a.tran_code as ''Tran Code'',
                a.mapped_amount as ''Mapped Amount'',
                b.deposit_date as ''Deposit Date'',
